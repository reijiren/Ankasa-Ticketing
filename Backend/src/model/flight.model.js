...
const flightModel = {
    ...
    detailFlight: (id_flight) => {
        return new Promise((resolve, reject) => {
            db.query(`select * from flight join airline on flight.airline = airline.id_airline where id_flight='${id_flight}';`, (err, res) => {
                if(err){
                    reject(err);
                }
                resolve(res);
    ...
    findFlight: (limit, page, sortBy, sortOrd, data) => {
        ...
        return new Promise((resolve, reject) => {
            db.query(`select * from flight join airline on flight.airline = airline.id_airline where 
            ${ data.price ?
                `(price between ${data.price.min} and ${data.price.max}) ${counter < max ? addCount() : ' '}`
                : ''
            }
            ${data.direct ? 
            ...
            ${data.wifi ?
                `wifi = 1 ${counter < max ? addCount() : ''}`
                : ''
            }
            ${data.depMidnight ?
                `(time_departure >= '00:00:00' and time_departure <= '05:59:59' ${counter < max ? addCount() : ''}`
                : ''
            }
            ${data.depMorning ?
                `(time_departure >= '06:00:00' and time_departure <= '11:59:59' ${counter < max ? addCount() : ''}`
                : ''
            }
            ${data.depAfternoon ?
                `(time_departure >= '12:00:00' and time_departure <= '17:59:59' ${counter < max ? addCount() : ''}`
                : ''
            }
            ${data.depNight ?
                `(time_departure >= '18:00:00' and time_departure <= '23:59:59' ${counter < max ? addCount() : ''}`
                : ''
            }
            ${data.arrMidnight ?
                `(time_arrived >= '00:00:00' and time_arrived <= '05:59:59' ${counter < max ? addCount() : ''}`
                : ''
            }
            ${data.arrMorning ?
                `(time_arrived >= '06:00:00' and time_arrived <= '11:59:59' ${counter < max ? addCount() : ''}`
                : ''
            }
            ${data.arrAfternoon ?
                `(time_arrived >= '12:00:00' and time_arrived <= '17:59:59' ${counter < max ? addCount() : ''}`
                : ''
            }
            ${data.arrNight ?
                `(time_arrived >= '18:00:00' and time_arrived <= '23:59:59' ${counter < max ? addCount() : ''}`
                : ''
            }
            ${data.airlineName ? 
            ...
            order by ${sortBy} ${sortOrd} limit ${limit} offset ${offset}`, (err, res) => {
                if(err){
                    reject(err);
                }
                resolve(res);
    ...
    deleteFlight: (id_flight) => {
        return new Promise((resolve, reject) => {
            db.query(`delete from flight where id_flight='${id_flight}'`, (err, res) => {
                if(err){
                    reject(err);
                }
                resolve(res);
